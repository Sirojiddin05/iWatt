...
import 'package:i_watt_app/features/charging_processes/data/models/transaction_model.dart';
...
abstract class TransactionHistoryDatasource {
  Future<GenericPagination<TransactionModel>> getTransactionHistory({String next = ''});
...
class TransactionHistoryDatasourceImpl implements TransactionHistoryDatasource {
  ...
  @override
  Future<GenericPagination<TransactionModel>> getTransactionHistory({String next = ''}) async {
  ...
  Future<TransactionMessageEntity> getSingleTransaction(int transactionId) async {
    try {
      ...
      if (response.statusCode! >= 200 && response.statusCode! < 300) {
        try {
          return TransactionMessageModel.fromJson(response.data);
        } catch (e) {
          print('Error parsing transaction message: $e');
          throw ParsingException(errorMessage: e.toString());
        }
      } else {
        final error = GenericErrorModel.fromJson(response.data);
