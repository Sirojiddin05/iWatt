                    ...
                    showCupertinoModalBottomSheet(
                      ...
                      builder: (ctx) {
                        return BlocProvider(
                          ...
                          child: ChargingPaymentCheck(
                            body: BlocBuilder<TransactionHistoryBloc, TransactionHistoryState>(
                              builder: (context, state) {
                                if (state.getSingleTransactionHistoryStatus.isInProgress) {
                                  return const Center(child: CircularProgressIndicator.adaptive());
                                }
                                if (state.getSingleTransactionHistoryStatus.isSuccess) {
                                  return SingleChildScrollView(
                                    padding: const EdgeInsets.all(16),
                                    child: ChequeWidget(cheque: state.singleTransactionHistory),
                                  );
                                }
                                if (state.getSingleTransactionHistoryStatus.isFailure) {
                                  return Center(
                                    child: Text(
