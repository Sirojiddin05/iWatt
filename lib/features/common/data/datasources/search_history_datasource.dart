...
abstract class SearchHistoryDataSource {
  Future<SearchHistoryModel> postSearchHistory(int locationId);

  Future<GenericPagination<SearchHistoryModel>> getSearchHistory();
...
class SearchHistoryDataSourceImpl extends SearchHistoryDataSource {
  ...
  @override
  Future<SearchHistoryModel> postSearchHistory(int locationId) async {
    try {
      final response = await _dio.post('common/HistorySearchedLocation/', data: {"location": locationId});
  ...
  @override
  Future<GenericPagination<SearchHistoryModel>> getSearchHistory() async {
    try {
      final response = await _dio.get('common/HistorySearchedLocation/');
      if (response.statusCode! >= 200 && response.statusCode! < 300) {
        return GenericPagination.fromJson(
            response.data, (p0) => SearchHistoryModel.fromJson(p0 as Map<String, dynamic>));
      } else {
        final error = GenericErrorModel.fromJson(response.data);
        throw ServerException(
          statusCode: error.statusCode,
  ...
  Future<void> deleteAllSearchHistory() async {
    try {
      final response = await _dio.delete('common/HistorySearchedLocation/');
      if (!(response.statusCode! >= 200 && response.statusCode! < 300)) {
        final error = GenericErrorModel.fromJson(response.data);
        throw ServerException(
          statusCode: error.statusCode,
  ...
  Future<void> deleteSingleSearchHistory(int id) async {
    try {
      final response = await _dio.delete('common/HistorySearchedLocation/$id/');
      if (!(response.statusCode! >= 200 && response.statusCode! < 300)) {
        final error = GenericErrorModel.fromJson(response.data);
        throw ServerException(
          statusCode: error.statusCode,
